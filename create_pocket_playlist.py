...
# 3. DELETE LIST (Including your specific request)
BAD_KEYWORDS = ["fashion", "overseas", "yupp", "usa", "pluto", "sun nxt", "sunnxt", "jio specials hd", "zee devotional"]
...
# === RECURSIVE COOKIE FINDER (Proven to work) ===
...
# === JIO HOTSTAR FETCHER (Multi-Lang + Clean Titles) ===
def fetch_jio_hotstar_live():
    ...
    try:
        ...
        # Clean cookie: remove quotes and braces so it is a raw string
        ...
        for event in events:
            ...
            for lang_code, lang_name in processed_langs:
                # Construct URL with Pipe | syntax
                # Using QUOTES for headers as originally requested
                stream_url = (
                    f'{JIO_BASE_STREAM}?id={vid_id}&lang={lang_code}&{JIO_UID_PASS}'
                    f'|Cookie="{cookie_val}"&User-Agent="{JIO_UA}"&Referer="{JIO_REF}"'
                )
                
                # Title: Put [Language] at the start for visibility
